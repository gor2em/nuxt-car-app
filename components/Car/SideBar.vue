...
<template>
    <div class="shadow border w-64 mr-10 z-30 h-[190px]">
        ...
        <div class="p-5 flex justfiy-between relative cursor-pointer">
            <h3>Price</h3>
            <h3 class="text-blue-400 capitalize" @click="updateModal('price')">{{ priceRangeText }}
            </h3>

            <div class="absolute border shadow left-56 p-5 top-1 -m-1 bg-white" v-if="modal.price">
                <input class="border p-1 rounded mb-1" type="number" placeholder="Min" v-model="priceRange.min">
                <input class="border p-1 rounded" type="number" placeholder="Max" v-model="priceRange.max">

                <button class="bg-blue-400 w-full mt-2 rounded text-white py-1" @click="onChangePrice">Apply</button>
            </div>
        </div>
        <!--PRICE END-->
